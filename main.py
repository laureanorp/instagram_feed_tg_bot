...
from database_functions import (
    update_last_post_date_in_db,
    get_followed_profiles_from_db,
    remove_followed_profile_from_db,
    update_update_interval_in_db,
    profile_is_already_followed,
    add_followed_profile_to_db,
...
if not all([TELEGRAM_TOKEN, INSTA_USERNAME, INSTA_PASSWORD]):
    raise ValueError(
        "Please set the TELEGRAM_TOKEN, INSTA_USERNAME and INSTA_PASSWORD environment variables."
    )
...
)  # TODO - maybe is better to use a self-made shortener to avoid rate limits
...
async def check_profiles_for_updates(context):
    ...
    try:
        ...
        if not followed_profiles:
            logging.info(
                f"No profiles followed by user (chat_id): {chat_id}. Checking loop will stop."
            )
            return
        for followed_profile in followed_profiles:
            profile = Profile.from_username(
                loader.context, followed_profile["username"]
            )
            ...
            if new_posts:
                ...
                update_last_post_date_in_db(
                    chat_id, followed_profile["username"], new_posts[0].date
                )
            else:
                logging.info(f"No new posts found for {followed_profile['username']}")
            # Schedule the next check
            update_interval_hours = collection.find_one({"_id": chat_id})[
                "update_interval_hours"
            ]
            context.job_queue.run_once(
                check_profiles_for_updates, update_interval_hours * 3600, data=chat_id
            )
...
async def shorten_and_send_post(
    post: Post, chat_id: int, followed_profile: dict
) -> None:
...
async def follow_new_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        message = (
            "⚠️ Please provide the username, for example: /follow_user PedroPascal20"
        )
    ...
    followed_profile = {
        ...
        - timedelta(days=4),  #  get_last_post_date(username),
    }
...
async def unfollow_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        message = (
            "⚠️ Please provide the username, for example: /unfollow_user PedroPascal20"
        )
        await context.bot.send_message(chat_id=chat_id, text=message)
        return
    # If there's no user entry or the user is not following any profiles, return a message
    if (
        collection.count_documents({"_id": chat_id}) == 0
        or collection.find_one({"_id": chat_id})["followed_profiles"] == []
    ):
        await context.bot.send_message(
            chat_id=chat_id, text="You are not following any profiles."
        )
        return
...
async def get_current_profiles_followed(
...
if __name__ == "__main__":
    try:
        ...
        configure_update_interval = CommandHandler(
            "configure_update_interval", configure_update_interval
        )
        profiles_followed_handle = CommandHandler(
            "profiles_followed", get_current_profiles_followed
        )
